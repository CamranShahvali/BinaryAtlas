...
[[nodiscard]] const Instruction* nextInstruction(const DisassemblyResult& disassembly,
                                                 Address address)
{
...
[[nodiscard]] const Instruction* previousInstruction(const DisassemblyResult& disassembly,
                                                     Address address)
{
...
[[nodiscard]] bool isBoundaryLike(const Instruction* instruction)
{
  ...
  return instruction->flow_kind == InstructionFlowKind::return_flow ||
         ...
         instruction->mnemonic == "int3" || instruction->mnemonic == "nop";
}

[[nodiscard]] bool matchesFramePrologue(const Instruction* first, const Instruction* second,
                                        const Instruction* third)
{
  ...
  return first->mnemonic == "endbr64" && second->mnemonic == "push" &&
         second->operand_text == "rbp" && third != nullptr && third->mnemonic == "mov" &&
         third->operand_text == "rbp, rsp";
}

[[nodiscard]] std::vector<SeedRecord> collectSeeds(const BinaryImage& image,
                                                   const DisassemblyResult& disassembly,
                                                   bool enable_prologue_heuristics)
{
  std::map<Address, std::vector<SeedRecord>> by_address;

  auto add_seed = [&](Address address, std::string source, std::string label, double confidence,
                      int priority, bool hard_boundary)
  {
    if (!image.isExecutableAddress(address) || disassembly.find(address) == nullptr)
    {
      return;
    }
    by_address[address].push_back(
        {{address, std::move(source), std::move(label), confidence}, priority, hard_boundary});
  };
  ...
  {
    if (instruction.flow_kind == InstructionFlowKind::call_direct &&
        instruction.branch_target.has_value())
    {
      add_seed(*instruction.branch_target, "call_target",
               generatedFunctionName(*instruction.branch_target), 0.7, 2, false);
    }
  }
  ...
  if (enable_prologue_heuristics)
  {
    for (const Instruction& instruction : disassembly.instructions())
    {
      ...
      const Instruction* third =
          second == nullptr ? nullptr : nextInstruction(disassembly, second->address);
      ...
      add_seed(instruction.address, "prologue", generatedFunctionName(instruction.address), 0.55, 3,
               false);
    }
  }
  ...
  for (auto& [address, records] : by_address)
  {
    std::sort(records.begin(), records.end(),
              [](const SeedRecord& left, const SeedRecord& right)
              {
...
[[nodiscard]] std::map<Address, std::vector<FunctionSeed>>
seedReasonMap(const std::vector<SeedRecord>& seeds)
{
...
[[nodiscard]] std::string resolveFunctionName(Address entry,
...
collectReferencedStrings(const RecoveredFunction& function, const DisassemblyResult& disassembly,
  ...
  for (const BasicBlock& block : function.blocks)
  {
    for (Address address : block.instruction_addresses)
    {
      ...
      for (const MemoryReference& reference : instruction->memory_references)
      {
        ...
        {
          const auto string_size_bytes = checkedAdd(candidate.value.size(), std::size_t{1});
          const auto string_size = string_size_bytes.has_value()
                                       ? checkedIntegralCast<Address>(*string_size_bytes)
                                       : std::nullopt;
          const auto end =
              string_size.has_value() ? checkedAdd(candidate.address, *string_size) : std::nullopt;
          if (!end.has_value())
          {
            continue;
          }
...
[[nodiscard]] RecoveredState
recoverFunction(Address entry, const std::vector<FunctionSeed>& reasons, const BinaryImage& image,
                const DisassemblyResult& disassembly, const std::set<Address>& hard_boundary_seeds,
                const std::map<Address, std::string>& symbol_names)
{
    ...
    while (current != nullptr)
    {
      if (!block.instruction_addresses.empty() && block_starts.contains(current->address))
      {
        ...
        block.outgoing_edges.push_back(
            {block.start, current->address, BlockEdgeType::fallthrough, true});
      ...
      case InstructionFlowKind::call_indirect:
      {
        ...
        if (current->branch_target.has_value() &&
        ...
        if (next != nullptr)
        {
          ...
          block.outgoing_edges.push_back(
              {block.start, next_address, BlockEdgeType::call_fallthrough, true});
        }
        else
        {
          state.function.partial = true;
      ...
      case InstructionFlowKind::jump_conditional:
      {
        block.end = next_address;
        if (current->branch_target.has_value() &&
            disassembly.find(*current->branch_target) != nullptr &&
            image.isExecutableAddress(*current->branch_target))
        {
        ...
        if (next != nullptr)
        {
          ...
          pending.insert(next_address);
          block.outgoing_edges.push_back(
              {block.start, next_address, BlockEdgeType::false_branch, true});
        }
        else
        {
          state.function.partial = true;
      ...
      case InstructionFlowKind::jump_unconditional:
      {
            ...
            image.isExecutableAddress(*current->branch_target))
        {
          ...
          block.outgoing_edges.push_back(
              {block.start, *current->branch_target, BlockEdgeType::jump, !boundary_jump});
          if (!boundary_jump)
          {
            block_starts.insert(*current->branch_target);
            pending.insert(*current->branch_target);
      ...
      case InstructionFlowKind::trap:
        block.end = next_address;
        block.outgoing_edges.push_back(
            {block.start, std::nullopt, BlockEdgeType::return_edge, false});
        current = nullptr;
        break;
      }
    }
  ...
  std::sort(state.function.blocks.begin(), state.function.blocks.end(),
            [](const BasicBlock& left, const BasicBlock& right)
            { return left.start < right.start; });

  state.function.callees.assign(callees.begin(), callees.end());
  std::sort(state.function.call_sites.begin(), state.function.call_sites.end(),
...
[[nodiscard]] std::vector<CallGraphEdge>
buildCallGraph(const BinaryImage& image, const std::vector<RecoveredFunction>& functions)
{
  ...
  for (const RecoveredFunction& function : functions)
  {
    for (const CallSite& call_site : function.call_sites)
    {
      ...
      {
        if (unique_edges.emplace(function.entry, *call_site.target, false, iterator->second->name)
                .second)
        {
      ...
      const auto symbol_iterator = std::find_if(
          image.symbols().begin(), image.symbols().end(), [&call_site](const Symbol& symbol)
          { return symbol.address == *call_site.target && !symbol.name.empty(); });
      if (symbol_iterator != image.symbols().end())
      {
        if (unique_edges.emplace(function.entry, *call_site.target, true, symbol_iterator->name)
                .second)
        {
  ...
  std::sort(edges.begin(), edges.end(),
            [](const CallGraphEdge& left, const CallGraphEdge& right)
            {
...
Result<ProgramAnalysis> ProgramAnalyzer::analyze(const BinaryImage& image,
                                                 const DisassemblyResult& disassembly,
                                                 const AnalysisOptions& options) const
{
  ...
  for (const SeedRecord& seed : seeds)
  {
    ...
    RecoveredState recovered =
        recoverFunction(seed.seed.address, reasons.at(seed.seed.address), image, disassembly,
                        hard_boundary_seeds, symbol_names);
